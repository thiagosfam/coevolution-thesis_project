...
def co_evolve(resistance_ea, spy_ea, 
              ...
              p_cross=0.1, std=0.1, hof_size=10):
    """
    ...
    spy_pop = spy_ea.population
     ...
     # 1. Evaluate resistance population against spy population
    print("Evaluating resistance population...")
    resistance_with_fitness = resistance_ea.evaluate_population(
            resistance_pop, resistance_pop, spy_pop, 'resistance'
    )
    resistance_HoF.append(sorted(resistance_with_fitness, key=lambda x: x[1], reverse=True)[0])
        
    # 2. Evaluate spy population against resistance population
    print("Evaluating spy population...")
    spy_with_fitness = spy_ea.evaluate_population(
        spy_pop, spy_pop, resistance_pop, 'spy'
    )
    spy_HoF.append(sorted(spy_with_fitness, key=lambda x: x[1], reverse=True)[0])
    ...
    for generation in range(num_gen):
        ...
        resistance_offspring_with_fitness = resistance_ea.evaluate_population(
            resistance_offspring, resistance_offspring, spy_pop, 'resistance'
        )
        
        # 6. Evaluate spy offspring against resistance population
        print("Evaluating spy offspring...")
        spy_offspring_with_fitness = spy_ea.evaluate_population(
            spy_offspring, spy_offspring, resistance_pop, 'spy'
        )
...
def visualize_coevolution(resistance_stats, spy_stats, save_path=None):
    ...
    ax1.set_title('Co-Evolution of Resistance Fighters and Spies', fontsize=16)
...
def export_populations(resistance_pop, spy_pop, resistance_HoF, spy_HoF, results_dir="results"):
    ...
    resistance_csv_path = os.path.join(results_dir, "coevolution_final_resistance_population.csv")
    ...
    spy_csv_path = os.path.join(results_dir, "coevolution_final_spy_population.csv")
    ...
    resistance_hof_csv_path = os.path.join(results_dir, "coevolution_resistance_hall_of_fame.csv")
    ...
    spy_hof_csv_path = os.path.join(results_dir, "coevolution_spy_hall_of_fame.csv")
...
def export_ancestral_lines(resistance_ancestral_line, spy_ancestral_line, results_dir="results"):
    ...
    df_resistance = pd.DataFrame(resistance_data)
    resistance_csv_path = os.path.join(results_dir, "coevolution_resistance_ancestral_line.csv")
    df_resistance.to_csv(resistance_csv_path, index=False)
    ...
    df_spy = pd.DataFrame(spy_data)
    spy_csv_path = os.path.join(results_dir, "coevolution_spy_ancestral_line.csv")
    df_spy.to_csv(spy_csv_path, index=False)
...
if __name__ == "__main__":
    ...
    visualize_coevolution(resistance_stats, spy_stats, save_path="results/coevolution.png") 