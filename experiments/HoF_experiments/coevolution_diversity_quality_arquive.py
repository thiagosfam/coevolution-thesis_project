...
from evolutionary.archive_policy import QualityDiversityArchive
...
def co_evolve(resistance_ea, spy_ea, 
    ...
    """
    # Initialize Archive for both populations
    resistance_archive = QualityDiversityArchive(max_size=100)
    spy_archive      = QualityDiversityArchive(max_size=100)
    ...
    # Seed resistance Archive with best individual from initial population
    resistance_archive.update(best_initial_resistance, best_initial_resistance_fitness, generation=0)
    ...
    # Seed spy Archive with best individual from the initial population
    spy_archive.update(best_initial_spy, best_initial_spy_fitness, generation=0)
    ...
    for generation in range(1, num_gen + 1):
        ...
        for ind in resistance_offspring:
            opponents = spy_archive.sample_opponents(k=2) # 2 spy opponents
            fitness = resistance_ea._evaluate_individual(individual=ind, 
                                                         role='resistance', 
                                                         opponents=opponents)
        ...
        for ind in spy_offspring:
            opponents = resistance_archive.sample_opponents(k=3) # 3 resistance opponents
        ...
        resistance_archive.update(best_resistance, best_resistance_fitness, generation=generation)
        ...
        # Update spy Archive
        spy_archive.update(best_spy, best_spy_fitness, generation=generation)
    ...
    return resistance_pop, spy_pop, resistance_archive, spy_archive, resistance_stats, spy_stats, resistance_best_of_gen, spy_best_of_gen
...
if __name__ == "__main__":
    ...
    base_dir = os.path.join("results", "quality_diversity_archive")
    ...
    for run_idx in range(1, evolutionary_runs + 1):
        ...
        print("########## Testing Diversity Quality Archive ##########")
        ...
        final_resistance, final_spies, resistance_archive, spy_archive, resistance_stats, spy_stats, resistance_best_of_gen, spy_best_of_gen = co_evolve(
        ...
        # Export Archives
        resistance_archive.export(filepath=os.path.join(run_dir, "resistance_archive.csv"))
        spy_archive       .export(filepath=os.path.join(run_dir, "spy_archive.csv"))
