...
from evolutionary.pareto_archive import LayeredParetoArchive, make_play_fn
...
def co_evolve(resistance_ea, spy_ea, 
    ...
    resistance_archive = LayeredParetoArchive(max_layers=5)
    spy_archive        = LayeredParetoArchive(max_layers=5)

    # Seed the archives
    resistance_archive.learners = resistance_pop.copy()
    resistance_archive.testers  = spy_pop.copy()

    spy_archive.learners = spy_pop.copy()
    spy_archive.testers  = resistance_pop.copy()

    # Define function utilized for discrimination in the pareto archive
    play_spy = make_play_fn(role='spies',     n_games=30)
    play_res = make_play_fn(role='resistance', n_games=30)
    ...
    for ind in resistance_pop:
        fitness = resistance_ea._evaluate_individual(individual=ind, 
                                                    role='resistance',
                                                    opponent_pool=resistance_archive.testers)
    ...
    for ind in spy_pop:
        fitness = resistance_ea._evaluate_individual(individual=ind, 
                                                    role='spy',
                                                    opponent_pool=spy_archive.testers)
    ...
    for generation in range(1, num_gen + 1):
        ...
        resistance_archive.update(learner_candidates=resistance_pop,
                                  tester_candidates=spy_pop,  
                                  play_fn=play_res,
                                  role='resistance'
        )
        ...
        # Update spy Archive
        spy_archive.update(learner_candidates=spy_pop,
                           tester_candidates=resistance_pop,  
                           play_fn=play_spy,
                           role='spies'
        )
...
if __name__ == "__main__":
    ...
    base_dir = os.path.join("results", "LAPCA")
    ...
    for run_idx in range(1, evolutionary_runs + 1):
        ...
        final_resistance, final_spies, resistance_archive, spy_archive, resistance_stats, spy_stats, resistance_best_of_gen, spy_best_of_gen = co_evolve(
            ...
            num_gen=3, # Number of generations
