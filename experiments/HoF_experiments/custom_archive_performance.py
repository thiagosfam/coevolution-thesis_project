...
from evolutionary.archive_policy import CustomArchivePerformance, Individual
...
def co_evolve(resistance_ea, spy_ea, 
    ...
    resistance_archive = CustomArchivePerformance(role='resistance', max_size=100, threshold=0.6)
    spies_archive = CustomArchivePerformance(role='spies', max_size=100, threshold=0.6)
   ...
   # Seed Archive with best 10 individuals from initial populations
    ...
    initial_resistance = [Individual(genes=resistance[0], i_fitness=resistance[1], generation=0) for resistance in top_10_resistance]
    initial_spies = [Individual(genes=spy[0], i_fitness=spy[1], generation=0) for spy in top_10_spies]
    
    resistance_archive.seed(batch=initial_resistance,
                            opponent_batch=initial_spies)
    
    spies_archive.seed(batch=initial_spies,
                       opponent_batch=initial_resistance)
    ...
    for generation in range(1, num_gen + 1):
        ...
        for ind in resistance_offspring:
            opponent_pool = spies_archive.get_archive()
            fitness = resistance_ea._evaluate_individual(individual=ind, 
                                                         role='resistance', 
        ...
        for ind in spy_offspring:
            opponent_pool = resistance_archive.get_archive()
        ...
        resistance_candidate = Individual(genes=best_resistance,
                                          i_fitness=best_resistance_fitness,
                                          generation=generation)
                                          
        resistance_archive.add_or_replace(cand=resistance_candidate,
                                          opponent_archive=spies_archive)
        # Update spies Archive
        spy_candidate = Individual(genes=best_spy,
                                   i_fitness=best_spy_fitness,
                                   generation=generation)
        
        spies_archive.add_or_replace(cand=spy_candidate,
                                     opponent_archive=resistance_archive)
    ...
    return resistance_pop, spy_pop, resistance_archive, spies_archive, resistance_stats, spy_stats, resistance_best_of_gen, spy_best_of_gen
...
if __name__ == "__main__":
    ...
    base_dir = os.path.join("results", "CustomArchivePerformance100")
    ...
    for run_idx in range(1, evolutionary_runs + 1):
        ...
        final_resistance, final_spies, resistance_archive, spies_archive, resistance_stats, spy_stats, resistance_best_of_gen, spy_best_of_gen = co_evolve(
        ...
        # Export Archives
        resistance_archive.export(
            csv_path=os.path.join(run_dir, "resistance_archive.csv"))
        
        spies_archive.export(csv_path=os.path.join(run_dir, "spy_archive.csv"))
