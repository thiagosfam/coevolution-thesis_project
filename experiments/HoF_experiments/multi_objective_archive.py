...
def co_evolve(resistance_ea, spy_ea, 
    ...
    archive =ArchiveManager(max_size=100)
   ...
   # Seed Archive with best 10 individuals from initial populations
    ...
    initial_resistance = [Individual(genes=resistance[0], generation=0, fitness=resistance[1], role='resistance') for resistance in top_10_resistance]
    initial_spies = [Individual(genes=spy[0], generation=0, fitness=spy[1], role='spy') for spy in top_10_spies]
    
    for ind in initial_resistance:
        archive.add(individual=ind)
    ...
    for generation in range(1, num_gen + 1):
        ...
        for ind in resistance_offspring:
            opponent_pool = archive.get_genomes(role='spy')
            fitness = resistance_ea._evaluate_individual(individual=ind, 
                                                         role='resistance', 
        ...
        for ind in spy_offspring:
            opponent_pool = archive.get_genomes(role='resistance')
        ...
        # Add new resistance to Archive
        resistance_candidate = Individual(
            genes=best_resistance,
            generation=generation,
            fitness=best_resistance_fitness,
            role='resistance')
                                          
        # Update  new spy to Archive
        spy_candidate = Individual(
            genes=best_spy,
            generation=generation,
        ...
        # Add to archives
        archive.add(resistance_candidate)
        archive.add(spy_candidate)
        
        # If archive is full, trigger evaluation + pruning
        if len(archive.get_archive("resistance")) > archive.max_size or \
        len(archive.get_archive("spy")) > archive.max_size:

            archive.cross_evaluate()
            archive.compute_novelty(role='resistance', k=3)
            archive.compute_novelty(role='spy', k=3)

            archive.prune(method='multiobjective', role='resistance')
            archive.prune(method='multiobjective', role='spy')

            print(f"[Archive] Pruned after generation {generation}")
    ...
    return resistance_pop, spy_pop, archive, resistance_stats, spy_stats, resistance_best_of_gen, spy_best_of_gen
...
if __name__ == "__main__":
    ...
    base_dir = os.path.join("results", "MultiObjectiveArchive")
    os.makedirs(base_dir, exist_ok=True)

    evolutionary_runs = 11
    

    for run_idx in range(10, evolutionary_runs + 1):
        ...
        final_resistance, final_spies, archive, resistance_stats, spy_stats, resistance_best_of_gen, spy_best_of_gen = co_evolve(
        ...
        # Export Archives
        archive.export_archive(role='resistance', csv_path=os.path.join(run_dir, "resistance_archive.csv"))
        archive.export_archive(role='spy', csv_path=os.path.join(run_dir, "spy_archive.csv"))
