...
def co_evolve(resistance_ea, spy_ea, 
    ...
    for generation in range(num_gen):
        ...
        if len(spy_HoF) < 2:
            opponent_cooplayer_pool = spy_pop
        else:
            opponent_cooplayer_pool = spy_hof_players    

        print("Evaluating resistance offspring...")
        resistance_offspring_with_fitness = resistance_ea.evaluate_population_HoF(
            population=resistance_offspring, opponent_pool_hof=opponent_cooplayer_pool, role='resistance'
        )
        
        # 6. Evaluate spy offspring against resistance population
        if len(resistance_HoF) < 3:
            opponent_cooplayer_pool = resistance_pop
        else:
            opponent_cooplayer_pool = resistance_hof_players
        ...
        spy_offspring_with_fitness = spy_ea.evaluate_population_HoF(
            population=spy_offspring, opponent_pool_hof=opponent_cooplayer_pool, role='spy'
        )
...
def export_stats(resistance_stats, spy_stats, results_dir="results"):
    ...
    pd.DataFrame({
        ...
        'best_individuals': resistance_stats['best_individuals']
    ...
    pd.DataFrame({
        ...
        'best_individuals': spy_stats['best_individuals']
...
if __name__ == "__main__":
    ...
    for i in range(evolutionary_runs):
        ...
        print(f"Running evolutionary run {i+1} of {evolutionary_runs}")
        ...
        final_resistance, final_spies, resistance_HoF, spy_HoF, resistance_stats, spy_stats, resistance_ancestral_line, spy_ancestral_line = co_evolve(
            ...
            num_gen=200,  # 200 generations for longer evolution
        ...
        export_stats(resistance_stats, spy_stats, results_dir=f"results/run_{i+1}")
    
        # Export the populations and Hall of Fame
        export_populations(final_resistance, final_spies, resistance_HoF, spy_HoF, results_dir=f"results/run_{i+1}")
    
        # Export ancestral lines
        export_ancestral_lines(resistance_ancestral_line, spy_ancestral_line, results_dir=f"results/run_{i+1}")
    
        # Visualize the co-evolution statistics
        visualize_coevolution(resistance_stats, spy_stats, save_path=f"results/run_{i+1}/coevolution_HoF.png") 
